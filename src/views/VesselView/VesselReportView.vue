...
const { lastVoyageNo, nextVoyageNo } = storeToRefs(store);
...
const { isSuccess, isFetching, data: voyages } = store.getAllReports(props.imo);

const voyageDetails = {}; // uuid : arr of voyage details
const reports = {}; // uuid : arr of reports

// for (let i = 0; i < voyages.value.length; i++) {
//   const uuid = voyages.value[i].uuid;
//   const json = voyages.value[i].reports;
//   reports[uuid] = [];
//   voyageDetails[uuid] = {};
...
//   voyageDetails[uuid]["cur_voyage_no"] = voyages.value[i].voyage_num;
...
//   if (i === voyages.value.length - 1) {
//     // update voyage number
//     lastVoyageNo.value = voyages.value[i].voyage_num;
//   }
// }
