<template>
  <div class="grid lg:grid-cols-2 bg-white rounded-lg p-5 gap-4 shadow-card">
    ...
    <div class="col-span-2 grid grid-cols-10 border">
      <div
        class="col-span-10 lg:col-span-2 text-blue-700 p-3 border-b lg:border-b-0 lg:border-r bg-gray-50 text-14"
      >
        {{ $t("wind") }}
      </div>
      <div
        class="col-span-4 lg:col-span-2 text-blue-700 p-3 border-b lg:border-b-0 border-r bg-gray-50 text-14"
      >
        {{ $t("direction") }}
      </div>
      <input
        ...
        class="col-span-6 lg:col-span-2 p-3 pl-4 border-b lg:border-b-0 lg:border-r bg-white text-14 text-gray-700 focus:outline-0"
      />
      <div
        class="col-span-4 lg:col-span-1 text-blue-700 p-3 border-r bg-gray-50 text-14"
      >
        {{ $t("force") }}
      </div>
      <select
        v-model="wave_force"
        class="col-span-6 lg:col-span-3 p-3 text-14 focus:border-0"
    ...
    <div class="col-span-2 lg:col-span-1 grid grid-cols-5 border">
      ...
      <div class="flex col-span-3 p-2 pl-4 border-l bg-white">
        <input
          v-model="max_wave_ht"
          @keypress="preventNaN($event, max_wave_ht)"
          placeholder="00.0"
          class="w-24 text-14 text-gray-700 focus:outline-0"
        />
...
const {
  ...
  heavyMaxWaveHt: max_wave_ht,
  heavyWaveForce: wave_force,
} = storeToRefs(store);
