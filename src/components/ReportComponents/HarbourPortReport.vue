<template>
	<div class="grid grid-cols-2 bg-white rounded-lg p-5 gap-4 shadow-card">
        ...
        <div class="col-span-2 flex space-x-5 text-gray-700 text-14">
            ...
            <div class="flex align-center space-x-2">
                <input type="radio" id="noonInHarbourPort" value="1" v-model="data.type">
                <label for="noonInHarbourPort">{{ $t("noonInHarbourPort") }}</label>
            </div>
        </div>

		<div class="col-span-2 lg:col-span-1 grid grid-cols-5 border text-14">
            <div class="col-span-2 text-blue-700 p-3 border-b border-r bg-gray-50">{{ $t("status") }}</div>
            ...
            >
                <option selected disabled value="default">{{ $t("selectEvent") }}</option>
            </select>
			<div class="col-span-2 text-blue-700 p-3 border-r border-b bg-gray-50">{{ $t("dateAndTime") }}</div>
			<DatePicker 
			...
			<div class="col-span-2 text-blue-700 p-3 border-r bg-gray-50">{{ $t("distanceTravelled") }}</div>
            <div class="flex col-span-3 p-2 pl-4 bg-white">
                <input v-model="data.distance_travelled" @keypress="preventNaN($event, data.distance_travelled)" placeholder="0" class="w-16 text-14 text-gray-700 focus:outline-0"/>
                <MiniUnitDisplay>NM</MiniUnitDisplay>
            </div>
		</div>

		<div class="col-span-2 lg:col-span-1 grid grid-cols-5 border bg-gray-50 text-14">
			<div class="col-span-2 row-span-2 text-blue-700 p-3 border-r">{{ $t("operations") }}</div>
			<div class="col-span-3 bg-white flex flex-col space-y-2 p-3 text-gray-700">
				<!-- TODO: make dynamic -->
				<div class="flex align-center space-x-2">
					<input type="checkbox" id="waiting" value="0" v-model="data.operations">
					<label for="waiting">{{ $t("waiting") }}</label>
				</div>
				<div class="flex align-center space-x-2">
					<input type="checkbox" id="cargoOperation" value="1" v-model="data.operations">
					<label for="cargoOperation">{{ $t("cargoOperation") }}</label>
				</div>
				<div class="flex align-center space-x-2"> 
					<input type="checkbox" id="bunkeringDebunkering" value="2" v-model="data.operations">
					<label for="bunkeringDebunkering">{{ $t("bunkeringDebunkering") }}</label>
				</div>
				<div class="flex align-center space-x-2">
					<input type="checkbox" id="others" value="3" v-model="data.operations">
		...
		<div class="col-span-2 lg:col-span-1 grid grid-cols-5 border bg-gray-50 text-14">
			<span class="col-span-2 row-span-3 text-blue-700 p-3 self-center">{{ $t("latitude") }}</span>
		...
		<div class="col-span-2 lg:col-span-1 grid grid-cols-5 border bg-gray-50 text-14">
			<span class="col-span-2 row-span-3 text-blue-700 p-3 self-center">{{ $t("longitude") }}</span>
