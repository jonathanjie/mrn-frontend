<template>
    <div class="flex h-20 items-center justify-between rounded-xl min-w-max bg-gray-50">
        <img v-if="report_type=='DEPART'" src="@/assets/icons/depart_report.svg" class="h-10 w-10 m-5"/>
        <img v-if="report_type=='ARRIVAL'" src="@/assets/icons/arrival_report.svg" class="h-10 w-10 m-5"/>
        <img v-if="report_type=='NOON'" src="@/assets/icons/noon_report.svg" class="h-10 w-10 m-5"/>
        <img v-if="report_type=='BUNKER'" src="@/assets/icons/bunker_report.svg" class="h-10 w-10 m-5"/>
        ...
        <img v-if="report_type=='DEPART'" src="@/assets/icons/depart_icon.svg" class="m-5 mr-[60px]"/>
        <img v-if="report_type=='ARRIVAL'" src="@/assets/icons/arrival_icon.svg" class="m-5 mr-20"/>
        <img v-if="report_type=='NOON'" src="@/assets/icons/noon_icon.svg" class="m-5 mr-[86px]"/>
        <img v-if="report_type=='BUNKER'" src="@/assets/icons/bunker_icon.svg" class="m-5"/>
        <img src="@/assets/icons/detail.svg" class="w-9 h-9 mr-5"/>   
    </div>
</template>
...
export default {
    ...
    props: {
        ...
        status: {
            type: String, // TODO: enum - Anchoring/Sailing/...
            required: false,
            default: "STATUS"
        },
        cargold: {
        ...
        distance_to_go: {
            type: String, // TODO: enum - change to Number after backend
            required: false,
            default: '-'
        },
        date_of_submission: {
